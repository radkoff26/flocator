...
internal class MarkFragmentViewModel @Inject constructor(
    ...
    private fun loadMark() {
        ...
        compositeDisposable.add(
            mainRepository.getMark(markId!!, userId!!)
    ...
    private fun loadAuthorData() {
        _markLiveData.value!!
        compositeDisposable.add(
            mainRepository.getUser(_markLiveData.value!!.mark.authorId)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                    {
    ...
    fun toggleLike() {
        ...
        if (_markLiveData.value!!.mark.hasUserLiked) {
            unlikeMark()
            compositeDisposable.add(
                mainRepository.unlikeMark(markId!!, userId!!)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                        {
            ...
            likeMark()
            compositeDisposable.add(
                mainRepository.likeMark(markId!!, userId!!)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                        {
